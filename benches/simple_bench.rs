...
use graviplex::simulation::{
    Simulation, BarnesHutGravityStrategy, NaiveGravityStrategy,
    KdTreeCollision, NaiveCollisionStrategy, NoCollisionStrategy,
};
...
fn bench_simulation_strategies(c: &mut Criterion) {
    ...
    group.bench_function("barnes_hut_10000", |b| {
        let mut sim = Simulation::new();
        sim.set_gravity_strategy(Box::new(BarnesHutGravityStrategy::new(0.5, 0.01)));
        sim.generate_bodies(10000);
...
fn bench_quadtree_only(c: &mut Criterion) {
    use graviplex::simulation::spatial::{Quadtree, Quad};
